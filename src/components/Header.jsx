...
function Header() {
    ...
    const getActiveLink = useCallback(() => {
        if (isReleases) return releasesRef.current;
    ...
    }, [isDocs, isReleases, getActiveLink]);
    ...
    return (
      <header className="bg-black/30 backdrop-blur-xl text-white text-sm uppercase font-sans flex items-center justify-between border-b border-white/5 relative">
        ...
        <button
          className="md:hidden px-4 py-4 text-white/80 hover:text-white transition-colors"
        ...
        <div className="hidden md:flex relative" ref={navRef}>
          <div
            className="hidden md:block absolute bottom-0 h-[0.5px] bg-white"
            style={{
          ...
          <Link
            ...
            className={`px-4 py-4 transition-colors font-light ${
              !isDocs && !isReleases ? "text-white" : "text-white/60 hover:text-white"
            }`}
          >
            README
          </Link>
          ...
          <Link
            ...
            to="/releases"
            className={`px-4 py-4 transition-colors font-light ${
              isReleases ? "text-white" : "text-white/60 hover:text-white"
            }`}
          >
            RELEASES
          </Link>
        ...
        {isMenuOpen && (
          <div className="absolute top-full left-0 w-full md:hidden bg-black/95 backdrop-blur-xl border-b border-white/5 z-50">
            <Link
              to="/"
              onClick={() => setIsMenuOpen(false)}
              className={`block px-6 py-4 transition-colors font-light ${
                !isDocs && !isReleases ? "" : "text-white/60 hover:text-white hover:bg-white/5"
              }`}
            >
              README
            </Link>
            <Link
              ...
              onClick={() => setIsMenuOpen(false)}
              className={`block px-6 py-4 transition-colors font-light ${
                isDocs ? "" : "text-white/60 hover:text-white hover:bg-white/5"
              }`}
            >
              GET STARTED
            </Link>
            <Link
              ...
              onClick={() => setIsMenuOpen(false)}
              className={`block px-6 py-4 transition-colors font-light ${
                isReleases ? "" : "text-white/60 hover:text-white hover:bg-white/5"
              }`}
            >
              RELEASES
            </Link>
            <a
              ...
              rel="noopener noreferrer"
              onClick={() => setIsMenuOpen(false)}
              className="flex items-center gap-2 px-6 py-4 text-black bg-white/90 hover:bg-white transition-colors font-light"
            >
              ...
              GITHUB
              {starCount != null && (
                <span className="ml-auto text-sm tabular-nums opacity-80">
                  ★ {starCount.toLocaleString()}
                </span>
              )}
            </a>
